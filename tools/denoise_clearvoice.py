...
if __name__ == '__main__':
    ...
    for s1_fp in tqdm(wav_fp_list):
        ...
        s1_avg_logprob = [seg.avg_logprob for seg in segments][0]  # 这个list里所有的avg_logprob是一样的
        ...
        s2_avg_logprob = [seg.avg_logprob for seg in segments][0]  # 这个list里所有的avg_logprob是一样的

        if s1_avg_logprob > s2_avg_logprob:
            res_fp_list.append(s1_fp)
        else:
            print(f">>> detect unexpected denoise result, add s2 as final result (s2: {s2_fp})")
            res_fp_list.append(s2_fp)

        # print(f"lang_prob: {'s1' if s1_lang_prob>s2_lang_prob else 's2'} s1={s1_lang_prob} s2={s2_lang_prob}")
        # print(f"logprob: {'s1' if s1_avg_logprob>s2_avg_logprob else 's2'} s1={s1_avg_logprob} s2={s2_avg_logprob}")
        # Audio(s1_fp)
        # Audio(s2_fp)
