...
# 暂时无解，人工空白音的音量是0，而自动切分的rms阈值是0.02，所以自动切分会早于人工空白音，比如
# 1. 0~6s是自动切分，
# 2. 6~6.5s是一句话的尾音
# 3. 6.5~8.5s是添加的人工空白音
# 这就导致自动切分认为第一句话在0~6s结束了，第二句话从6s开始直到N秒，这第二句话里就会包含人工空白音
def step_slice(inp_dir, out_dir, threshold=-34, min_length=4000, min_interval=100):
    # threshold = -34  # 音量小于这个值视作静音的备选切割点，公式是10^(x/20)如果要用静音点就试试-200，其结果是1e-10基本是0
